...
(function($){
	...
	$.Goods_Collect_Result = function(result,weebox,form)
	{
		...
		if(result.status == 1)
		{
			$('.PUB_IMG',form).show().append(result.item);
			...
			file.onchange = function(){
				...
				FANWE.Pic_Upload_Obj = $.ajaxFileUpload(
				{
					...
					success:function(result,status){
						...
						if(result.status == 1)
						{
							$('.PUB_IMG',form).show().append(result.html);
							$.Pub_Share_Tags(form);
						}
						else if(result.status == 0)
						{
	...
	$.Upload_Photo_Complete = function(status,html,server)
	{
		if(status == 1)
		{
			...
			$('.PUB_IMG',FANWE.UPLOAD_PHOTO_REFELE_FORM).show().append(html);
	...
	{
		if($('.PUB_IMG li',form).length == 0 || SETTING.share_is_tag == 0)
		{
...
jQuery(function($){
	$("img.LOAD_SCALE_IMG").each(function(){
		...
		var oldImg = this;
		var img= new Image();
		img.src = this.getAttribute('scaleSrc');
		...
		{
			oldImg.src = this.getAttribute('scaleSrc');
			delete img;
			return;
		}
		...
		var autoScale = function(){
			...
			oldImg.width = width;
			oldImg.height = height;
			oldImg.src = img.src;
			var left = (parent.width() - width) / 2;
			var top = (parent.height() - height) / 2;
			$(oldImg).css({"margin-top":top,"margin-left":left});
			delete img;
		};
